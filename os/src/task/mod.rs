...
impl TaskManager {
    ...
    fn find_next_task(&self) -> Option<usize> {
        ...
        (current + 1..current + self.num_app + 1)
            ...
            .find(|id| inner.tasks[*id].task_info.is_ready())
    }
    ...
    fn update_current_task_info(&self, syscall_id: usize) -> isize {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current]
            .task_info
            .update_task_info(syscall_id, get_time_ms());
        0
    }
}
...
pub unsafe fn connect_to_current_task(ti: *mut TaskInfo) {
    let inner = TASK_MANAGER.inner.exclusive_access();
    let current = inner.current_task;
    *ti = inner.tasks[current].task_info;
}
